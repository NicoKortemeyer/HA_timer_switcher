...
class TestScheduleArbitraryAction:
    def test_toggle_action(self):
        s = make_schedule(action="toggle")
        assert s.action == "toggle"
        assert Schedule.from_dict(s.to_dict()).action == "toggle"

    def test_media_play_action(self):
        s = make_schedule(entity_id="media_player.living_room", action="media_play")
        assert s.action == "media_play"
        assert Schedule.from_dict(s.to_dict()).action == "media_play"

    def test_open_cover_action(self):
        s = make_schedule(entity_id="cover.garage", action="open_cover")
        assert Schedule.from_dict(s.to_dict()).action == "open_cover"

    def test_arbitrary_string_action(self):
        s = make_schedule(action="any_custom_service")
        assert Schedule.from_dict(s.to_dict()).action == "any_custom_service"
...
class TestPresetSerialisation:
    ...
    def test_preset_arbitrary_action(self):
        p = Preset(name="y", entity_id="cover.x", action="open_cover", mode="delay")
        assert Preset.from_dict(p.to_dict()).action == "open_cover"
